...
class ScanViewModel(val commonRepository: CommonRepository,
                    val context: Context,
                    val activity: Activity,val fitpassScanListener:FitpassScanListener): ViewModel(), HandleResponseListeners {
    ...
    fun getScanData(requestBody: JSONObject) {
    ...
    override fun handleSuccess(response1: Response<JsonObject?>, api: String?) {
        ...
        var gson = Gson()
        var response: FitpassScanResponse = gson.fromJson(jsonObject.toString(), FitpassScanResponse::class.java)
